 ...
 */
@Component(service = JcrUtilService.class, immediate = true, property = { "process.label=ANZ - JCR Service",
		Constants.SERVICE_DESCRIPTION + "=ANZ - JCR Service", Constants.SERVICE_VENDOR + "=ANZ",
...
public class JcrUtilService {
	 ...
	 */
	private static final String SUBSERVICE = "readwriteservice";
	...
	public static Node createNode(Node parentNode, String newNodeName) {
		Node newNode = null;
		try {
			if (!parentNode.hasNode(newNodeName)) {
				newNode = parentNode.addNode(newNodeName, JcrConstants.NT_UNSTRUCTURED);
			...
			LOG.error("Error while adding node :: {}", e);
		}

		return newNode;
	}

	public static void setNodeProperty(Node node, String proprtyName, String propetyValue) {

		try {
			node.setProperty(proprtyName, propetyValue);
